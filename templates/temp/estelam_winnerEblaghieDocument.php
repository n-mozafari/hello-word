...
<div style = "min-width:100%; width:100%; min-height:500px; direction: rtl; margin:20px auto 20px auto; font-size: 12px;">
    ...
    <div>با سلام : در اجرای استعلام بهای شماره <?=$param['trade']['generalInfo']['data']['code']?> مورخ <?=isset($param['trade']['generalInfo']['data']['varchar_1']) && !empty($param['trade']['generalInfo']['data']['varchar_1']) ? str_replace('-' , '/', $jdf->g2j($param['trade']['generalInfo']['data']['varchar_1'])) : $jdf->jdate('Y/m/d')?> موضوع <?=$param['trade']['generalInfo']['data']['title']?> نظر به اینکه با پیشنهاد مبلغ کل <?=number_format($param['trade_subject']['winner']['data']['money'])?> ریال برنده شناخته شده اید لذا خواهشمند است دستور فرمایید ظرف مدت <?=$tahvil?> <?=$text?></div>
