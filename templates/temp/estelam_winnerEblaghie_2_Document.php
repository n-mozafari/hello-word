...
<div style = "min-width:100%; width:100%; min-height:500px; direction: rtl; margin:20px auto 20px auto; font-size: 12px;">
    ...
    <div> 
        ...
        <p>باسلام : در اجرای  استعلام بهای  شماره<?=$param['trade']['generalInfo']['data']['code']?> مورخ 
            <?=isset($param['trade']['generalInfo']['data']['varchar_1']) && !empty($param['trade']['generalInfo']['data']['varchar_1']) ? str_replace('-' , '/', $jdf->g2j($param['trade']['generalInfo']['data']['varchar_1'])) : $jdf->jdate('Y/m/d')?> ( موضوع  <?=$param['trade']['generalInfo']['data']['title']?> ) ، نظربه اينکه با مبلغ کل 
            <?=$param['trade_subject']['winner']['data']['money']?> (<?=numToString($param['trade_subject']['winner']['data']['money'])?>) ریال بشرح ذيل  وبا مد نظر قرار دادن شرايط ذکر شده در استعلام فوق برنده شناخته شده اید ، لذا خواهشمنداست نمايندگان قانونی ظرف مدت يك هفته از تاريخ ابلاغ اين نامه با به همراه داشتن تضمين حسن انجام تعهدات به ميزان <?=$param['trade']['specInfo']['data']['hosnAnjamTaahodatAmount']?>
    ...
    {?>
        <table cellspacing="0" cellpadding="0" width="100%" border="1">
            <tr>
                ...
                <td style="border:solid 1px; text-align: center;">شرح</td>
                <td style="border:solid 1px; text-align: center;">مقدار/ تعداد</td>
                <td style="border:solid 1px; text-align: center;">قيمت واحد( بريال )</td>
                <td style="border:solid 1px; text-align: center;">قيمت كل( بريال )</td>
                <!--<td style="border:solid 1px; text-align: center;">جمع کل مبلغ پيشنهادي با اعمال  <?=$param['trade']['generalInfo']['data']['arzeshAfzoode_total'] * 100?> درصد عوارض وماليات ارزش افزوده( به ريال)</td>    -->
            </tr>
            <?
            ...
            { 
                ...
                foreach($tfiArr as $value)
                {
                    ...
                    <tr>
                        ...
                        <td style="border:solid 1px; text-align: center;"><?=convertEnNum2FaNum($value['title'])?></td>
                        ...
                        <td style="border:solid 1px; text-align: center;"><?=convertEnNum2FaNum(number_format($value['mavad_basePrice'] * $value['quantity']))?></td>
                        <!--<td style="border:solid 1px; text-align: center;"><?=convertEnNum2FaNum(number_format($value['mavad_basePrice'] * $value['quantity']*(1 + $param['trade']['generalInfo']['data']['arzeshAfzoode_total'])))?></td>    -->
                    </tr>
                    <?
                }
            }
            ...
            ?>
            <tr>
                <td colspan="2">جمع كل</td>
                <td colspan="5"><?=convertEnNum2FaNum(number_format($param['trade_subject']['winner']['data']['money']))?> ریال</td>
            </tr>
            <tr>
                <td colspan="2">جمع كل با احتساب <?=$param['trade']['generalInfo']['data']['arzeshAfzoode_total'] * 100?> درصد  عوارض و مالیات بر ارزش افزوده</td>
                <td colspan="5"><?=convertEnNum2FaNum(number_format(($param['trade']['generalInfo']['data']['arzeshAfzoode_total'] + 1) * $param['trade_subject']['winner']['data']['money']))?> ریال</td>
            </tr>
