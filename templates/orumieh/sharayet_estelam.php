<?$jdf = new jdf()?>
<?
$girandegan = $param['trade']['girandegan']['data'];
foreach($girandegan as $ebRow)
{?>
<br/>
<div style = "min-width:90%; width:90%; min-height:870px; direction: rtl; border:2px solid; margin:20px auto 20px auto;">
    <div style="min-height:820px; margin: 20px;">
        ...
        <div style="clear: both; text-align: center; margin:20px auto 30px auto; font-size:15px;"><strong>استعلام بها شماره <?=$param['trade']['generalInfo']['data']['code']?></strong></div>
        <div style="text-align: right; margin:20px auto 30px auto; font-size:12px;"><strong>مدیریت محترم شرکت<?=$ebRow['title']?></strong></div>
        <div style="text-align: right; margin:20px auto 30px auto; font-size:12px;">
        <p>احتراما به اطلاع می رساند این شرکت در نظر دارد اقلام زیر را مطابق با مشخصات فنی، دستورالعمل، الزامات و معیارهای ارزیابی فنی شرکت توانیر خریداری نماید. خواهشمندد است قیمت پیشنهادی خود را در پاکت در بسته حداکثر تا <label style="font-weight: bold;">ساعت <?=''?> روز <?=''?> مورخه <?=''?></label> به آدرس <?=''?> تحویل نمایند. <label style="font-weight: bold;">کد اقتصادی فروشنده الزامی است.</label></p>
        <p></p>
        </div>
